    ...
    function(values, items, options, path) {
      ...
      if (items.length > 1) {
        ...
        values.forEach(function(value, i) {
          var p = (path ? path + '.' : '') + i;
          if (i < items.length) {
            self.addErrors(errors, items[i].validate(value, options, p));
          }
          else if (!self.additionalItems()) {
            self.addError(errors, 'Array index outside tuple length: ' + i, 'additionalItems', p);
          }
        });
      }
        ...
        values.forEach(function(value, i) {
          var p = (path ? path + '.' : '') + i;
          self.addErrors(errors, items[0].validate(value, options, p));
        });
      }
