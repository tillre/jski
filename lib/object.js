    ...
    function(value, reqs, options, path) {
      ...
      reqs.forEach(function(name) {
        ...
        if (!value.hasOwnProperty(name)) {
          self.addError(errors, 'Required property is missing: ' + name, 'required', p);
        }
      });
      return errors;
    }
  ...
  this.addConstraint(new Constraint(
    ...
    function(value, properties, options, path) {
      ...
      for (var n in value) {
        p = path ? path + '.' + n : n;
        if (properties[n]) {
          this.addErrors(errors, properties[n].validate(value[n], options, p));
        }
        else if (!this.additionalProperties()) {
          this.addError(errors, 'Object has no property with name: ' + n, 'additionalProperties', p);
        }
      }
      return errors;
    },
